...
public class EmployeeService {
	...
	public static void displayById(int pid) {
		int id=searchById(pid);
		if(id != -1)
			System.out.println(emparr[id]);
		else
	...
	public static double calculateBonusById(int pid){
		int id=searchById(pid);
		if(id != -1) {
			if(emparr[id] instanceof SalariedEmp) {
				return ((SalariedEmp)emparr[id]).calculateBonus();
			}
		}
		System.out.print("Not a Salesemp ");
		return 0.0;
	}
}
